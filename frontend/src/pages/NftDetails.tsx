...
import { API_URL, NFT_ADDRESS, MARKETPLACE_ADDRESS, NFT_ABI, MARKETPLACE_ABI } from "../config"
...
function NftDetails() {
  ...
  const handleBuy = async () => {
    ...
    try {
      ...
      const tx = await marketplace.buyItem(NFT_ADDRESS, nft.tokenId, { value: nft.price })
      await tx.wait()
  ...
  if (isLoading) return <div className="py-20 text-center text-on-surface-muted">Loading...</div>
  if (!nft) return <div className="py-20 text-center text-error">NFT not found</div>

  const isOwner = address?.toLowerCase() === nft.owner.toLowerCase()

  return (
    <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      <button onClick={() => navigate(-1)} className="mb-6 text-sm font-medium text-secondary hover:text-primary transition-colors">
        &larr; Back
      </button>
      ...
      <div className="grid gap-10 md:grid-cols-2">
        <div className="overflow-hidden rounded-2xl border border-border bg-surface p-4 shadow-sm">
          {nft.imageUrl ? (
            <img src={resolveIpfsUri(nft.imageUrl)} alt={nft.name || ""} className="w-full rounded-xl object-cover" />
          ) : (
        ...
        <div className="flex flex-col items-start">
          <h1 className="font-sora text-3xl font-bold text-on-surface mb-2">{nft.name || `NFT #${nft.tokenId}`}</h1>
          <p className="text-sm text-secondary mb-6">
            Owned by <span className="font-mono text-primary font-medium">{shortenAddress(nft.owner)}</span>
          </p>
          
          <div className="mb-6 w-full rounded-xl border border-border bg-surface p-6 shadow-sm">
            <h2 className="mb-2 text-xs font-semibold uppercase tracking-wider text-secondary">Description</h2>
            <p className="text-sm text-on-surface-muted leading-relaxed">{nft.description || "No description provided."}</p>
          </div>
          
          <div className="mb-6 w-full rounded-xl border border-border bg-surface p-6 shadow-sm">
            <h2 className="mb-4 text-xs font-semibold uppercase tracking-wider text-secondary">Current Status</h2>
            
            {nft.listed ? (
              <div className="mb-6">
                <p className="text-3xl font-mono font-bold text-on-surface">{formatEther(nft.price!)} ETH</p>
                <p className="text-xs font-medium text-secondary mt-1">Listed for sale</p>
              </div>
            ) : (
              <p className="mb-6 text-sm font-medium text-secondary">Not listed for sale</p>
            )}
            
            {!address ? (
              <p className="text-sm font-medium text-error bg-error/10 p-3 rounded-lg">Please connect your wallet to interact.</p>
            ) : isOwner ? (
              nft.listed ? (
                ...
                <div className="flex flex-col gap-3">
                  ...
                  />
                  <button onClick={handleList} disabled={isProcessing || !listPrice || Number(listPrice) <= 0} className="btn-primary w-full py-3">
              ...
              nft.listed ? (
                <button onClick={handleBuy} disabled={isProcessing} className="btn-primary w-full py-3">
                  {isProcessing ? "Processing..." : "Buy Now"}
                </button>
              ) : (
                <p className="text-sm text-secondary italic text-center w-full bg-background p-3 rounded-lg border border-border">This NFT is not for sale.</p>
              )
            )}
          </div>
        </div>
